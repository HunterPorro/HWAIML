...
model = XGBRegressor(random_state=42, n_jobs=-1, n_estimators=1000, max_depth=10, learning_rate=0.03, subsample=0.8, colsample_bytree=0.8)
...
for i in range(len(bins) - 1):
    ...
    if i == 0:
        m_range = np.linspace(0.8, 0.9, 100)
        o_range = np.linspace(1500, 2500, 100)
    elif i == 1:
        m_range = np.linspace(0.75, 0.85, 100)
        o_range = np.linspace(1500, 2500, 100)
    elif i == 2:
        m_range = np.linspace(0.82, 0.92, 100)
        o_range = np.linspace(0, 1000, 100)
    elif i == 3:
        m_range = np.linspace(0.75, 0.85, 100)
        o_range = np.linspace(-1000, 500, 100)
    elif i == 4:
        m_range = np.linspace(0.75, 0.85, 100)
        o_range = np.linspace(-1000, 0, 100)
    else:
        m_range = np.linspace(0.8, 0.95, 100)
        o_range = np.linspace(1000, 2500, 100)
